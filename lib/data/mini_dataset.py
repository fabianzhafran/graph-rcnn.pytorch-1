...
class MiniDataset(Dataset):
    def __init__(self, data_dir="/projectnb/llamagrp/shawnlin/ref-exp-gen/dataset/mini/1", transform=None):
        assert os.path.exists(data_dir), \
            "cannot find folder {}, please download refcoco data into this folder".format(data_dir)

        self.data_dir = data_dir
        self.img_paths = glob.glob(data_dir+"/*.jpg")
        self.img_ids = [re.sub(r".*\/(.*)\.jpg", "\\1", p) for p in self.img_paths]
